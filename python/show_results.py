...
pixels = []
with open(os.path.join(script_dir, '../data/output_image.txt'), "r") as f:
    for line in f:
        val = line.strip()
        try:
...
original_img = Image.open(os.path.join(script_dir, '../images/resized_input.jpg')).convert('L').resize((100, 100))
